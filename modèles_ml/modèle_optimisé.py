# -*- coding: utf-8 -*-
"""
Created on Sat Dec 13 10:22:14 2025
...
for i, step in enumerate(chain_steps):
    ...
    print(f"\n--- Analyse des variables clés pour : {target_col} ---")
    
    # Vérification que le modèle supporte l'importance des features (Cas des Random Forest)
    if hasattr(model, 'feature_importances_'):
        # 1. Récupération des importances
        importances = model.feature_importances_
        feature_names = X_train_current.columns
        
        # 2. Création d'un DataFrame propre et trié
        feature_imp_df = pd.DataFrame({
            'Variable': feature_names,
            'Importance': importances
        }).sort_values(by='Importance', ascending=False)
        
        # 3. Affichage textuel des Top 10
        print(f"Top 10 variables les plus influentes ({model_type}) :")
        print(feature_imp_df.head(10))
        
        
        plt.figure(figsize=(10, 6))
        # On affiche les 15 meilleures pour la lisibilité
        sns.barplot(x='Importance', y='Variable', data=feature_imp_df.head(15), palette='viridis')
        
        plt.title(f"Importance des Variables - Modèle pour {target_col}")
        plt.xlabel("Score d'Importance (Contribution au modèle)")
        plt.ylabel("Variables Biomécaniques")
        plt.tight_layout()
        plt.show()
        
    else:
        print("Ce type de modèle ne fournit pas nativement l'importance des features.")
